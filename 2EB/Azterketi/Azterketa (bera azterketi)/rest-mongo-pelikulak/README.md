# Datu basea pelikulak deitzen da, eta kolekzioa filmoteka
...
- MongoDB multi-document ACID transactions for 3 functions. See `MongoDBPelikulaRepository.saveAll()`.
- MongoDB Aggregation pipeline. See `MongoDBPelikulaRepository.getAverageAge()`.
- Implementation of basic CRUD queries. See `MongoDBPelikulaRepository.java`.
- MongoDB typed collection with automatic mapping to POJOs using codecs: See `ConfigurationSpring.java`.
- How to manipulate correctly ObjectidId across, the REST API, the POJOs and the database itself. See the main trick in `Pelikula.java`.
